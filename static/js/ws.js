...
ws.onmessage = function(evt)
{
    ...
    if (data.exception) {
        if (data.exception == 'Unauthenticated')
        {
           $(location).attr('href', '/auth');
        }
        $("#dialog_error").text(data.exception);
        $("#dialog_error").dialog('open');
    }
    else if (data.action == 'chdir')
    {
    ...
    {
        responseCreateFile(data.response);
    }
...
function responseShowFiles(data)
{
    ...
    if (data.dir != '/'){
        $('#files').append('<li id="parent" data-name=".."><img src="/static/img/mime/inode-directory.png">..</li>');
    ...
    if (data.files) {
        ...
        $.each(data.files, function (num, file) {
            $('#files').append('<li data-name="'+ file.name + '" data-type="'+ file.type +'" data-size="'+ file.size +
                ...
                '<img src="/static/img/mime/' + file.type + '.png" />' + file.name +
...
function responseCreateFile(data)
{
    if (data.error) {
        $("#newfile_error").text('Error: ' + data.error);
    } else {
